...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="bg-white shadow-lg rounded-lg p-8 w-full max-w-2xl">
        ...
        <div class="mb-6">
            ...
            {{-- @if($plantImage->diagnoses->isEmpty())
                <p class="text-gray-500 italic">No expert has confirmed a diagnosis yet.</p>
            @else
                <ul class="list-disc list-inside space-y-2 text-gray-700">
                    @foreach($plantImage->diagnoses as $diagnosis)
                        <li>
                            Confirmed: <span class="font-semibold text-green-800">{{ $diagnosis->disease->name }}</span>
                            <span class="text-sm text-gray-600"> by {{ $diagnosis->expert->name }}</span>
                        </li>
                    @endforeach
                </ul>
            @endif --}}
